...
test('optional currency', async () => {
	expect((await WasmIntl()).NumberFormat('de_DE', currencyProps()).format(4711.22)).toBe('4.711,22 €');
});
...
test('optional unknown currency', async () => {
	expect((await WasmIntl()).NumberFormat('de_DE', currencyProps('NAN')).format(4711.22)).toBe("4.711,22 NAN");
});

test('optional EUR currency', async () => {
	expect((await WasmIntl()).NumberFormat('de_DE', currencyProps('EUR')).format(4711.22)).toBe('4.711,22 €');
});

test('optional USD currency', async () => {
	expect((await WasmIntl()).NumberFormat('de_DE', currencyProps('USD')).format(4711.22)).toBe('4.711,22 $');
});

test.skip('if we are a memory pig on format', async () => {
	const instance = (await WasmIntl()).NumberFormat('de_DE', currencyProps('EUR'));
...
test('full turn intl vs wasm-intl', async () => {
	...
	expect(((intlStop - intlStart) / (cfStop - cfStart)) * 100).toBeGreaterThan(60);
});
...
test('reuse intl vs wasm-intl', async () => {
	...
	expect(((intlStop - intlStart) / (cfStop - cfStart)) * 100).toBeGreaterThan(1);
});

test.skip('if we are a memory pig on CurrencyFormatter', async () => {
